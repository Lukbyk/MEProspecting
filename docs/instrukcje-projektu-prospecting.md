...
- **Analityk** włada od `sourced` do `selected` (budowa bazy). Tworzy kampanie i wykluczenia. **Twardy zakaz: nie rusza statusów od `sent` w górę.**
- **Operator** potwierdza wybór do wysyłki (`selected_for_outreach`), wysyła maile, decyduje o odpowiedziach.
...
- **Pliki per kampania** (czytelne artefakty obok bazy):
  ```
  kampanie/<id>-<slug>/
    hipoteza.md     (Strateg)      maile/     (Autor)
  ...
  ```
  Dziś rutynowo pisze pliki głównie **Mentor** (przeglądy); reszta podfolderów wypełnia się z czasem lub praca jest konwersacyjna.
...
- **Adapter skrzynki (Warstwa 2)** — Prowadzący/Kwalifikator mają zaprojektowany kontrakt (utwórz draft / wykryj wysłane / wykryj odpowiedź), ale skrzynka nie jest spięta. Do tego czasu działa **adapter ręczny**: Operator mówi, co wysłał (Prowadzący odnotowuje), i wkleja odpowiedzi. Dosłowna treść korespondencji do briefu też stąd. Spięcie skrzynki to temat przy aplikacji (Claude Code), nie przy skillach.
